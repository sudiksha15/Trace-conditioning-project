...
% Usually No only for day 4 yes as something wrong with indSounds
...
if ~isempty(sounds) && strcmp(fixed_tp_window, 'No')
    ...
    if strcmp(raw_behavior_type, 'ABF')
        ...
        windSize = 10;
        frac = 0.2;%;0.2 %0.05; %Can use 0.05 if session doesn't have tone-puff switch during session
    end
...
elseif strcmp(behav_select, 'None') %Excel Behavior
    indSoundOn = find(findPulses(binSounds) == 1);
    if strcmp(drop_artifact, 'Yes')
        indSoundOn = indSoundOn(indSoundOn ~= 19554);
    end
