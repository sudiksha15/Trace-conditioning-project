%Assume that ACSAT is all ready added to path before running
...
%% Pre-Processing
tic;
%Open Figure in current directory and get image data from it
open('full_projection_m_f_max_min_batch.fig');
ax = gca();
imgDiff = double(ax.Children.CData);
...
%% Save Output
CellList = all_roi;
save('ACSAT_Autorun_ROIs.mat', 'CellList')